...
// - Adjusted "roundRolling" handling to transition smoothly to roulette without "Spinning..." text.
// - Ensured pot items and visuals are cleared correctly for new rounds.
// - Retained previous profile modal, chat, and other functionalities.
...
const CONFIG = {
    ...
    WINNER_DISPLAY_DURATION: 7000, // Time winner info is shown before auto-reset
    TRADE_POPUP_DURATION: 15000, // Time winner trade offer popup is shown
    CONFETTI_COUNT: 150,
    EASE_OUT_POWER: 5,
...
const DOMElements = {
    ...
    jackpot: {
        ...
        // For hiding elements during roulette
    ...
    roulette: {
        ...
        returnToJackpotButton: document.getElementById('returnToJackpot'), // This button seems unused in HTML
    ...
    // NEW: Elements for Winner Trade Offer Popup
    winnerTradeOfferPopup: {
        popup: document.getElementById('winnerTradeOfferPopup'), // Will be added to index.html
        closeButton: document.getElementById('winnerTradeOfferPopupClose'), // Will be added
        acceptButton: document.getElementById('winnerTradeOfferPopupAccept'), // Will be added
        message: document.getElementById('winnerTradeOfferPopupMessage') // Will be added
    }
};

let currentUser = null;
let currentRound = null; // This will hold the state of the current round
...
// NEW: Show Winner Trade Offer Popup
function showWinnerTradeOfferPopup(offerURL, offerId) {
    ...
    if (!popup || !message || !acceptButton || !closeButton) {
        console.error("Winner trade offer popup elements not found.");
        // Fallback to a standard notification if popup elements are missing
        showNotification(
    ...
    acceptButton.onclick = () => { // Remove previous listener before adding new one
    ...
    closeButton.onclick = () => { // Remove previous listener
        popup.classList.remove('show');
        if (winnerTradeOfferPopupTimeout) clearTimeout(winnerTradeOfferPopupTimeout);
    };
...
function updateDepositButtonState() {
    ...
    } else if (isSpinning || (currentRound && currentRound.status === 'rolling')) { // Check isSpinning for client-side animation, and 'rolling' for server state
        disabled = true;
...
function updateRoundUI() {
    ...
    if (!timerActive && !isSpinning && currentRound.status !== 'rolling') { // Only update timer directly if not actively managed or spinning
...
function updateTimerUI(timeLeft) {
    ...
    if (currentRound && currentRound.status === 'active' && !timerActive && currentRound.participants?.length === 0 && !isSpinning) {
        displayValue = CONFIG.ROUND_DURATION.toString();
    } else if ((timerActive || (currentRound && currentRound.status === 'active')) && timeToShow > 0 && !isSpinning) {
        displayValue = timeToShow.toString();
    } else if (isSpinning || (currentRound && currentRound.status === 'rolling' && !jackpotHeader.classList.contains('roulette-mode'))) {
        // If actively spinning (roulette animation), or server says 'rolling'
        // AND the header isn't ALREADY in roulette-mode (meaning switchToRouletteView hasn't fully run)
        // we want to avoid showing "Rolling" if the user wants a direct transition.
        // The actual roulette view is handled by switchToRouletteView.
        // If roundRolling event directly triggers switchToRouletteView, this text might not even show.
        displayValue = ""; // Keep blank or minimal text before visual roulette starts
    } else if (currentRound && (currentRound.status === 'completed' || currentRound.status === 'error')) {
        displayValue = "Ended";
    } else if (!timerActive && timeToShow <= 0 && currentRound && currentRound.status === 'active' && !isSpinning) {
        displayValue = "0";
    } else if (currentRound && currentRound.status === 'pending') {
        displayValue = "Waiting";
    } else if (!currentRound && !isSpinning) {
        displayValue = "--";
    }
...
function handleNewDeposit(data) {
    ...
    if (participantIndex !== -1) {
        currentRound.participants[participantIndex] = {
            ...currentRound.participants[participantIndex],
            itemsValue: data.itemsValue, // Backend sends total value for user, so overwrite
...
function startClientTimer(initialTime = CONFIG.ROUND_DURATION) {
    ...
    roundTimer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            updateDepositButtonState();
            // Server will send 'roundRolling' then 'roundWinner'
        }
    }, 1000);
}
...
function handleWinnerAnnouncement(data) {
    ...
    // updateTimerUI(0); // Make sure timer shows 0 or is blanked before switching
    currentRound.status = 'rolling'; // Reflect server state for UI consistency

    switchToRouletteView(); // This function now handles hiding header elements
    setTimeout(() => {
        startRouletteAnimation({ winner: winnerDetails });
    }, 300); // Short delay for view switch CSS animations
}

// MODIFIED: switchToRouletteView to better handle transition
function switchToRouletteView() {
    ...
    if (!header || !rouletteContainer || !jackpotHeaderValueDisplay || !jackpotHeaderTimerDisplay || !jackpotHeaderStatsDisplay) {
        console.error("Missing UI elements for roulette view switch.");
        return;
    }

    // Hide jackpot header elements by adding class
    [jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay].forEach(el => {
        el.classList.add('hide-for-roulette');
    });

    header.classList.add('roulette-mode'); // Adjust header styling for roulette (e.g., height)
    
    // Show and animate in roulette container
    rouletteContainer.classList.add('active'); // This class now handles display:flex, opacity, transform for fade/slide in
...
function handleSpinEnd(winningElement, winner) {
    ...
    if (winnerInfoBox && winnerAvatar && winnerName && winnerDeposit && winnerChance) {
        ...
        winnerInfoBox.classList.add('show'); // Use class to trigger animation/display
...
// MODIFIED: resetToJackpotView to ensure proper UI reset
function resetToJackpotView() {
    ...
    if (!header || !rouletteContainer || !winnerInfoBox || !track || !jackpotHeaderValueDisplay || !jackpotHeaderTimerDisplay || !jackpotHeaderStatsDisplay) {
    ...
    rouletteContainer.classList.remove('active'); // Hide roulette
    winnerInfoBox.classList.remove('show'); // Hide winner info box
    winnerInfoBox.style.display = 'none'; // Ensure it's fully hidden
    ...
    setTimeout(() => {
        ...
        [jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay].forEach((el, index) => {
            el.classList.remove('hide-for-roulette'); // Show header elements
            // Force reflow for transition if needed, or rely on CSS for fade-in if already set up
            el.style.opacity = '0'; 
            setTimeout(() => { el.style.opacity = '1'; }, 50 + index * 50);
        });
        
        // Call to reset pot visuals, timer, etc. for a new round display
        // This will also request fresh round data if socket is connected
        ...
        if (socket?.connected) {
            ...
            // If not connected, ensure UI reflects a default empty state
             if (!currentRound || currentRound.status !== 'pending') { // if not already pending
...
// MODIFIED: initiateNewRoundVisualReset for robustness
function initiateNewRoundVisualReset() {
    console.log("Initiating visual reset for new/empty round display.");

    // Reset client-side currentRound data for items and participants
    if (currentRound) {
        currentRound.items = [];
        currentRound.participants = [];
        currentRound.totalValue = 0;
        currentRound.winner = null;
        currentRound.timeLeft = CONFIG.ROUND_DURATION; // Default time for a new round
        // currentRound.status will be updated by server (e.g. 'pending' then 'active')
    } else { // If currentRound is null, initialize a basic structure for UI updates
        currentRound = {
            items: [], participants: [], totalValue: 0, winner: null,
            timeLeft: CONFIG.ROUND_DURATION, status: 'pending', roundId: null, serverSeedHash: null
        };
    }
    ...
    if (container) {
        ...
        if (emptyMsg) { // Ensure empty message element exists
    ...
    userColorMap.clear();
    updateDepositButtonState(); // Update based on potentially new round state
    updateParticipantsUI(); // To ensure empty message is shown if needed
}
...
function setupSocketConnection() {
    ...
    socket.on('roundCreated', (data) => {
        ...
        currentRound = data; // Set currentRound to the fresh data from server
        initiateNewRoundVisualReset(); // Reset UI based on this new, empty round
        updateRoundUI(); // Update timer, pot value, etc.
        updateDepositButtonState();
        console.log("Client UI reset for new round.");
    });
    ...
    socket.on('roundRolling', (data) => {
        ...
        if (currentRound && currentRound.roundId === data.roundId) {
            ...
            // Don't set timer text to "Rolling". switchToRouletteView will handle UI changes.
            // updateTimerUI will be called by handleWinnerAnnouncement or subsequent roundData
            updateDepositButtonState();
            // The actual transition to roulette view is handled by handleWinnerAnnouncement -> switchToRouletteView
            // If client directly needs to hide elements here:
            const { jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay } = DOMElements.jackpot;
            [jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay].forEach(el => {
                el?.classList.add('hide-for-roulette');
            });
    ...
    socket.on('roundWinner', (data) => {
        ...
        if (currentRound && currentRound.roundId === data.roundId) {
            ...
            currentRound.status = 'rolling'; // Ensure status shows rolling during animation
            handleWinnerAnnouncement(data);
    ...
    socket.on('roundCompleted', (data) => {
        console.log('Round completed event received (after payout etc.):', data);
        if (currentRound && currentRound.roundId === data.roundId) {
        ...
        updateDepositButtonState();
        // Note: resetToJackpotView is typically called after winner display timeout,
        // or by 'roundCreated' for the next round.
    });
    ...
    socket.on('roundData', (data) => {
        ...
        const oldRoundStatus = currentRound?.status;
        currentRound = data; // Update client's knowledge of the round

        updateRoundUI(); // General UI update for pot value, participant count (not timer if active)
        updateDepositButtonState();

        // If the new data indicates a different state that requires visual change:
        if (currentRound.status === 'rolling' || (currentRound.status === 'completed' && oldRoundStatus !== 'completed')) {
             if (!isSpinning && currentRound.winner) {
                 console.log("Received roundData: Winner known, triggering animation/display.");
                 handleWinnerAnnouncement(currentRound); // data here is currentRound
             } else if (!isSpinning && currentRound.status === 'rolling' && !currentRound.winner) {
                 // If server says it's rolling, but client isn't spinning and has no winner yet
                 // Prepare the view for roulette, but wait for winner announcement
                  const { jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay } = DOMElements.jackpot;
                    [jackpotHeaderValueDisplay, jackpotHeaderTimerDisplay, jackpotHeaderStatsDisplay].forEach(el => {
                        el?.classList.add('hide-for-roulette');
                    });
                  DOMElements.jackpot.jackpotHeader?.classList.add('roulette-mode');
                  DOMElements.roulette.inlineRouletteContainer?.classList.add('active'); // Show empty track
                  if (DOMElements.jackpot.timerValue) DOMElements.jackpot.timerValue.textContent = ""; // Clear timer text
             } else if (!isSpinning && currentRound.status === 'completed' && !currentRound.winner){
                  console.log("Received roundData: Completed but no winner, resetting view.");
                  resetToJackpotView();
             }
            ...
            initiateNewRoundVisualReset(); // This sets timer to CONFIG.ROUND_DURATION visually
        ...
        // Re-render participant deposits based on the comprehensive roundData
        const container = DOMElements.jackpot.participantsContainer;
        if(container) {
            container.innerHTML = ''; // Clear previous items before re-rendering
            if (DOMElements.jackpot.emptyPotMessage && (!currentRound.participants || currentRound.participants.length === 0)) {
            ...
            if (currentRound.participants?.length > 0) {
                const validParticipants = currentRound.participants.filter(p => p.user && (p.user._id || p.user.id));
    ...
    socket.on('tradeOfferSent', (data) => {
         ...
         if (currentUser && data.userId === (currentUser._id || currentUser.id) && data.offerURL && data.type === 'winning') {
              // Call the new function to show the dedicated popup for winnings
              showWinnerTradeOfferPopup(data.offerURL, data.offerId);
         } else if (currentUser && data.userId === (currentUser._id || currentUser.id) && data.type === 'winning') {
             // Fallback if popup elements are missing (though showWinnerTradeOfferPopup handles this too)
...
function setupEventListeners() {
    ...
    window.addEventListener('click', (e) => {
        ...
        // Close winner trade offer popup if clicked outside (optional)
        const winnerPopup = DOMElements.winnerTradeOfferPopup.popup;
        if (winnerPopup && winnerPopup.classList.contains('show') && !winnerPopup.contains(e.target)) {
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (event.key === 'Escape') {
             if (profileModal?.style.display === 'flex') hideModal(profileModal);
                 ...
                 userProfile?.classList.remove('open');
                 userProfile?.focus();
             } else if (DOMElements.winnerTradeOfferPopup.popup?.classList.contains('show')) {
                 DOMElements.winnerTradeOfferPopup.popup.classList.remove('show');
                 if(winnerTradeOfferPopupTimeout) clearTimeout(winnerTradeOfferPopupTimeout);
             }
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize DOMElements for the winner trade offer popup after DOM is loaded
    ...
    initiateNewRoundVisualReset(); // Initial UI state for an empty/new round
...
console.log("main.js updated with trade offer popup, smoother roulette transition, and pot clearing fixes.");
